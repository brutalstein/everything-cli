...
impl RepositoryIndex {
    ...
    pub fn language_capability_report(
        &self,
        snapshot_id: &str,
    ) -> Result<LanguageCapabilityReport, RepoError> {
        self.ensure_snapshot(snapshot_id)?;
        let text_files = count(
            &self.connection,
        ...
        let tier1 = count(
            &self.connection,
            "SELECT COUNT(*) FROM snapshot_files WHERE snapshot_id=? AND file_kind='text' AND parser_key LIKE 'tree-sitter-%'",
            snapshot_id,
        )?;
        let tier2 = count(
            &self.connection,
            "SELECT COUNT(DISTINCT source_path) FROM ri2_build_targets WHERE snapshot_id=? AND source_path IS NOT NULL",
            snapshot_id,
        )?;
        let tier3 = count(
            &self.connection,
            "SELECT COUNT(DISTINCT source_path) FROM ri2_graph_edges WHERE snapshot_id=? AND evidence_class='semantic_resolved' AND source_path IS NOT NULL",
            snapshot_id,
        )?;
        let tier4 = count(
            &self.connection,
            "SELECT COUNT(DISTINCT path) FROM runtime_links WHERE snapshot_id=?",
            snapshot_id,
        )?;
        let fallback = count(
            &self.connection,
            "SELECT COUNT(*) FROM snapshot_files WHERE snapshot_id=? AND file_kind='text' AND language='other'",
            snapshot_id,
        )?;
    ...
    pub fn build_packages(&self, snapshot_id: &str) -> Result<Vec<BuildPackage>, RepoError> {
        ...
        let rows = statement.query_map([snapshot_id], |row| {
            Ok(BuildPackage {
                ...
                workspace_member: row.get::<_, i64>(5)? != 0,
            })
        })?;
    ...
    pub fn project_dependencies(
        &self,
        snapshot_id: &str,
    ) -> Result<Vec<ProjectDependency>, RepoError> {
        self.ensure_snapshot(snapshot_id)?;
        let mut statement = self.connection.prepare(
        ...
        let rows = statement.query_map([snapshot_id], |row| {
            Ok(ProjectDependency {
                source_package_id: row.get(0)?,
                target_name: row.get(1)?,
                target_package_id: row.get(2)?,
                dependency_kind: row.get(3)?,
                manifest_path: row.get(4)?,
            })
        })?;
        rows.collect::<Result<Vec<_>, _>>().map_err(RepoError::from)
        ...
        for hit in lexical.hits {
            hits.insert(
                ...
                Ri2RetrievalHit {
                    ...
                    capability_tier: file_capability_tier(
                        &self.connection,
                        snapshot_id,
                        &hit.path,
                    )?,
            ...
            let graph = self.graph_traverse(
                ...
                GraphDirection::Both,
                TraversalBudget {
                    max_depth: expansion_budget.max_depth.min(1),
                    ..expansion_budget
                },
            )?;
            for node in graph.nodes {
                ...
                entry
                    .why_relevant
                    .push(format!("bounded_graph_neighbor:{}", hit.path));
            }
    ...
    pub fn diff_snapshots(
        ...
        from_snapshot: &str,
        to_snapshot: &str,
    ) -> Result<RepositoryChangeSet, RepoError> {
        ...
        for (path, hash) in &from {
            match to.get(path) {
                None => {
                    ...
                    invalidated.push(file_node_id(path));
                }
                Some(next) if next != hash => {
        ...
        Ok(RepositoryChangeSet {
            from_snapshot: from_snapshot.to_owned(),
            to_snapshot: to_snapshot.to_owned(),
            added_paths: added,
    ...
    pub fn symbol_continuity(
        &self,
        repo_id: &str,
        from_snapshot: &str,
        to_snapshot: &str,
    ) -> Result<Vec<SymbolContinuity>, RepoError> {
        let mut statement = self.connection.prepare(
        ...
        for row in rows {
            ...
            output.push(SymbolContinuity {
                ...
                evidence_class: EvidenceClass::parse(&evidence)?,
...
pub(crate) fn load_edge(
    ...
    let row = rows.next()?.ok_or_else(|| {
        RepoError::Integrity("inserted RI2 edge could not be reloaded".to_owned())
    })?;
...
fn graph_node(
    ...
    snapshot_id: &str,
    node_id: &str,
) -> Result<Option<GraphNode>, RepoError> {
    ...
    row.map(|(kind, label, path, line, hash)| {
        Ok(GraphNode {
            node_id: node_id.to_owned(),
            kind: GraphNodeKind::parse(&kind)?,
            label,
            path,
            source_line: line,
            content_sha256: hash,
...
fn adjacent_edges(
    ...
    node_id: &str,
    direction: GraphDirection,
) -> Result<Vec<GraphEdge>, RepoError> {
    let (sql, both) = match direction {
        GraphDirection::Outgoing => (
            "SELECT edge_id,source_node_id,target_node_id,edge_kind,evidence_class,confidence_milli,producer_id,producer_version,source_path,source_line,environment_fingerprint,valid_from_snapshot,valid_until_snapshot FROM ri2_graph_edges WHERE snapshot_id=? AND source_node_id=? ORDER BY edge_id",
            false,
        ),
        GraphDirection::Incoming => (
            "SELECT edge_id,source_node_id,target_node_id,edge_kind,evidence_class,confidence_milli,producer_id,producer_version,source_path,source_line,environment_fingerprint,valid_from_snapshot,valid_until_snapshot FROM ri2_graph_edges WHERE snapshot_id=? AND target_node_id=? ORDER BY edge_id",
            false,
        ),
        GraphDirection::Both => (
            "SELECT edge_id,source_node_id,target_node_id,edge_kind,evidence_class,confidence_milli,producer_id,producer_version,source_path,source_line,environment_fingerprint,valid_from_snapshot,valid_until_snapshot FROM ri2_graph_edges WHERE snapshot_id=? AND (source_node_id=? OR target_node_id=?) ORDER BY edge_id",
            true,
        ),
    };
    let mut statement = connection.prepare(sql)?;
    let mut rows = if both {
        statement.query(params![snapshot_id, node_id, node_id])?
    } else {
        statement.query(params![snapshot_id, node_id])?
    };
    let mut output = Vec::new();
    while let Some(row) = rows.next()? {
        output.push(graph_edge_from_row(snapshot_id, row)?);
    }
    Ok(output)
}
...
fn graph_edge_from_row(snapshot_id: &str, row: &rusqlite::Row<'_>) -> Result<GraphEdge, RepoError> {
    Ok(GraphEdge {
        ...
        evidence: EdgeEvidence {
            evidence_class: EvidenceClass::parse(&row.get::<_, String>(4)?)?,
            confidence_milli: row.get(5)?,
            ...
            valid_from_snapshot: row.get(11)?,
            valid_until_snapshot: row.get(12)?,
        },
    })
}

fn file_capability_tier(
    connection: &Connection,
    snapshot_id: &str,
    path: &str,
) -> Result<CapabilityTier, RepoError> {
    let parser: Option<String> = connection.query_row(
        "SELECT parser_key FROM snapshot_files WHERE snapshot_id=? AND path=?",
    ...
    if precise {
        return Ok(CapabilityTier::Tier3PreciseSemantic);
    }
    ...
    if project {
        return Ok(CapabilityTier::Tier2Project);
    }
    if parser.is_some_and(|key| key.starts_with("tree-sitter-")) {
        Ok(CapabilityTier::Tier1Syntax)
    } else {
        Ok(CapabilityTier::Tier0Text)
    }
}
...
fn snapshot_file_hashes(
    connection: &Connection,
    snapshot_id: &str,
) -> Result<BTreeMap<String, Option<String>>, RepoError> {
    let mut statement = connection.prepare(
        "SELECT path,content_sha256 FROM snapshot_files WHERE snapshot_id=? ORDER BY path",
    )?;
    let rows = statement.query_map([snapshot_id], |row| {
