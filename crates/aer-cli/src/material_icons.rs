//! Source-backed Material Symbols for terminal presentation.
//!
//! The canonical visual assets are the vendored Google Material Symbols Rounded
//! SVGs under `assets/material-symbols/rounded/`. Terminals cannot render SVG
//! directly and a Material font may not be installed, so `compact` is a
//! terminal-safe semantic projection chosen from broadly supported Unicode.
//! The SVG identity remains verified by SHA-256; `EVERYTHING_ASCII=1` forces the
//! explicit ASCII fallback.
...
mod tests {
    ...
    #[test]
    fn vendored_material_assets_match_recorded_sha256_and_terminal_fallbacks_exist() {
